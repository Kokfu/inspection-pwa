...
try {
    ...
    Invoke-Checked { npx playwright test tests/manager-technicians.spec.ts tests/manager-dashboard-navigation.spec.ts tests/manager-upcoming-services.spec.ts tests/manager-app-auth-transitions.spec.ts tests/manager-navigation-request-count.spec.ts tests/manager-created-time.spec.ts tests/manager-final-report-navigation.spec.ts tests/new-service-visit-schedule.spec.ts tests/technician-home-ownership.spec.ts tests/manager-technician-services-done.spec.ts tests/connectivity-recovery.spec.ts tests/portable-user-workspace.spec.ts --workers=1 }
    Set-Location $repo
