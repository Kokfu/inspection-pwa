...
try {
    ...
    Invoke-Checked { node --import tsx --test --test-concurrency=1 src/routes/managerTechnicians.test.ts src/routes/managerCustomers.test.ts src/routes/managerTechnicians.integration.test.ts src/routes/managerCustomers.integration.test.ts src/jobs/serviceVisits.test.ts src/jobs/serviceVisits.integration.test.ts src/routes/customerCreation.integration.test.ts src/reports/finalServiceReport.test.ts src/routes/managerLocations.integration.test.ts src/routes/technicianOwnership.integration.test.ts src/routes/dryWetRiserInspectionsV7.test.ts src/routes/masterSystemInspectionsAutomaticSprinklerV7.test.ts src/routes/masterSystemInspectionsHydrantSummary.test.ts }
    ...
    Invoke-Checked { npx playwright test tests/manager-technicians.spec.ts tests/manager-dashboard-navigation.spec.ts tests/manager-upcoming-services.spec.ts tests/manager-app-auth-transitions.spec.ts tests/manager-navigation-request-count.spec.ts tests/manager-created-time.spec.ts tests/manager-final-report-navigation.spec.ts tests/new-service-visit-schedule.spec.ts tests/technician-home-ownership.spec.ts tests/connectivity-recovery.spec.ts tests/portable-user-workspace.spec.ts --workers=1 }
    Set-Location $repo
