...
const technicianColumns = `id::int AS id, username, is_active AS "isActive", created_at AS "createdAt"`;
...
export function createManagerTechniciansRouter(database: Pick<Pool, "query" | "connect"> = pool) {
  ...
  router.post("/manager/technicians", requireRole("admin"), async (request, response, next) => {
    ...
    try {
      ...
      const result = await client.query(`INSERT INTO users (username,password_hash,role,is_active) VALUES ($1,$2,'inspector',true) ON CONFLICT (username) DO NOTHING RETURNING ${technicianColumns}`, [username.trim(), passwordHash]);
