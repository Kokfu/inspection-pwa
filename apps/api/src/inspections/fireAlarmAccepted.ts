import { parseFireAlarmRowPreset, parseFireAlarmSystemDefinition, resolveFireAlarmControls, resolveFireAlarmV6Controls } from "./templates/fireAlarmDefinitionControls.js";
import { fireAlarmV6ContractSha256 } from "./evidence/fireAlarmV6Evidence.js";
...
export type StoredFireAlarmDetail = { responses: FireAlarmResponses; template:{id:string;code:"MFE-FSSR";version:number}; configuration:{revisionId:string;revisionNumber:number}; contract:{responseSchemaVersion:1|2;snapshotSchemaVersion:1|2;systemContractSha256:string|null} };
...
export function validateStoredFireAlarmDetail(row:R):StoredFireAlarmDetail|undefined{
  // V6 was accepted through the reservation/manifest transaction.  Its read
  // adapter remains strict and separate so V6 data cannot be parsed as V3.
  if (rec(row.inspectionSnapshot) && row.inspectionSnapshot.schemaVersion === 2) {
    ...
    try { controls = resolveFireAlarmV6Controls(snap.system.definition, snap.template.version as 6|7); } catch { return; }
    ...
    const isV7 = snap.template.version === 7;
    const finding = (value: unknown) => isV7 ? value === "not_good" || value === "complete_repair" : value === "poor";
    const validGroup = (group: unknown, definitions: readonly { key: string; result: { options: Array<{ value: string }> } }[]) => rec(group) && exact(group, definitions.map((item) => item.key)) && definitions.every((definition) => { const field = group[definition.key]; return rec(field) && exact(field,["result","remarks"]) && definitions.find((item) => item.key === definition.key)!.result.options.some((option) => option.value === field.result) && text(field.remarks,2000) && (!finding(field.result) || (field.remarks as string).trim()); });
    if (!validGroup(responses.chargerAndBatteries, controls.chargerAndBatteries) || !validGroup(responses.mainFunctionKeys, controls.mainFunctionKeys)) return;
    if (!responses.secondaryAlarmDeviceRows.every((row) => { if (!rec(row) || !Object.keys(row).every((key) => secondaryKeys.includes(key) || key === "fieldRemarks") || !secondaryKeys.every((key) => key in row) || !controls.secondaryAlarmDeviceRows.alarmBell.options.some((option) => option.value === row.alarmBell) || !controls.secondaryAlarmDeviceRows.manualCallPoint.options.some((option) => option.value === row.manualCallPoint)) return false; const remarks = "fieldRemarks" in row ? row.fieldRemarks : {}; return rec(remarks) && Object.entries(remarks).every(([key,value]) => (key === "alarmBell" || key === "manualCallPoint") && text(value,2000)) && (!finding(row.alarmBell) || text(remarks.alarmBell,2000,true)) && (!finding(row.manualCallPoint) || text(remarks.manualCallPoint,2000,true)); })) return;
  ...
  return {responses:parsed,template:{id:row.templateId as string,code:"MFE-FSSR",version:snap.template.version as number},configuration:{revisionId:row.configurationRevisionId as string,revisionNumber:snap.configuration.revisionNumber as number},contract:{responseSchemaVersion:1,snapshotSchemaVersion:1,systemContractSha256:null}};
}
