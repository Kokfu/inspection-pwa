...
export async function stageV7Evidence(request: Request, actorUserId: number) {
  ...
  try {
    ...
    if (!uuid.test(f.photoUuid) || !uuid.test(f.inspectionClientUuid) || !uuid.test(f.jobId) || (systemKey !== "co2_fire_extinguisher" && systemKey !== "wet_chemical" && systemKey !== "fire_alarm_detector" && systemKey !== "hydrant") || !uuid.test(f.masterTemplateId) || f.masterTemplateVersion !== "7" || !hash.test(f.contractSha256) || !hash.test(f.sha256) || !["camera", "gallery", "unknown"].includes(f.captureSource) || !canonicalUtc.test(f.capturedAt) || new Date(f.capturedAt).toISOString() !== f.capturedAt) throw new V7EvidenceError(400, "VALIDATION_ERROR", "V7 staged evidence identity or metadata is invalid");
