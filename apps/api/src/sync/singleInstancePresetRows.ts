...
export type ExpectedConfiguredPresetRow = {
  locationId: string;
  ordinal: number;
  sortOrder: number;
...
/** Shared configured-row identity logic for the V7 systems that have NO zone
 * dimension (`configuration.supportsZones = false`) and a single primary
 * instance: Smoke Ventilation (Fan Schedule) and Fire Intercom (Station
 * Schedule). Unlike Hydrant / Hose Reel / Riser there is no zone snapshot or
 * per-row location text to authenticate - only the fixed preset-row identity
 * (`locationId` + `ordinal` -> `assetReference`). Configured rows come before
 * technician rows. This carries no per-system contract content, so both systems
 * (and any future no-zone single-instance system) share it; each keeps its own
 * `validResponses` for the fields that ARE contract-specific. */
export function expectedConfiguredSingleInstanceRows(system: R): ExpectedConfiguredPresetRow[] | undefined {
  ...
  const expected: ExpectedConfiguredPresetRow[] = [];
...
export function configuredSingleInstanceRowsMatch(responses: unknown, expected: ExpectedConfiguredPresetRow[]) {
