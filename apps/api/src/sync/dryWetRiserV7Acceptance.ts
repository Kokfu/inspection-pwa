...
export async function acceptDryWetRiserV7Inspection(item: SyncItem, actorUserId?: number): Promise<SyncResult> {
  ...
  try {
    ...
    if ((manifest.length > 0 || reserved) && (!reserved || reserved.job_id !== payload.jobId || reserved.system_key !== "dry_wet_riser" || reserved.master_template_version_id !== payload.masterTemplate.id || reserved.master_template_version !== 7 || reserved.system_contract_sha256 !== contractSha256 || reserved.reserved_by_user_id !== String(actorUserId))) { await client.query("ROLLBACK"); result.failed.push(unavailable(payload.clientUuid)); return result; }
  ...
  } catch (error) { await client.query("ROLLBACK").catch(() => undefined); if (isDryWetRiserV7AcceptedEvidenceUniqueViolation(error)) result.failed.push(fail(payload.clientUuid, "EVIDENCE_CONFLICT", "V7 evidence was accepted concurrently for another location and needs attention")); else result.failed.push(fail(payload.clientUuid, "SERVER_ERROR", "Dry/Wet Riser V7 form could not be saved"));
