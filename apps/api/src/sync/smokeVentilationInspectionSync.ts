...
type ExpectedConfiguredSmokeVentilationRow = {
  locationId: string;
  ordinal: number;
  sortOrder: number;
...
/** Smoke Ventilation has no zone dimension (`configuration.supportsZones = false`),
 * so unlike Hydrant/Hose Reel/Riser there is no zone snapshot or per-row
 * location text to authenticate - only the fixed Fan Schedule identity
 * (`locationId` + `ordinal` -> `assetReference`). */
export function expectedConfiguredSmokeVentilationRows(system: R): ExpectedConfiguredSmokeVentilationRow[] | undefined {
  ...
  const expected: ExpectedConfiguredSmokeVentilationRow[] = [];
