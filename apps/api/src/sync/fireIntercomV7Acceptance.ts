...
import { configuredFireIntercomRowsMatch, expectedConfiguredFireIntercomRows } from "./fireIntercomInspectionSync.js";
...
/** Fire Intercom has no checklist sections and no header fields at all - the
 * only result on the page is the per-row Condition. Must stay identical to the
 * fire_intercom adapter in `v7EvidenceContracts.ts` and to
 * `fireIntercomRowColumns` on the web. */
const rowColumns = [["conditionResult", "condition"]] as const;

export const isFireIntercomV7AcceptedEvidenceUniqueViolation = (error: unknown) => record(error) && error.code === "23505" && typeof error.constraint === "string" && v7AcceptedEvidenceUniqueConstraints.has(error.constraint);
...
function rowValues(definition: unknown) {
  ...
  const section = definition.sections.find((value) => record(value) && value.key === "station_schedule");
  const block = record(section) && Array.isArray(section.blocks) ? section.blocks.find((value) => record(value) && value.key === "station_schedule_rows") : undefined;
...
function validResponses(responses: Value, rows: Map<string, Set<string>>) {
  ...
  if (!exact(responses, ["schemaVersion", "rows", "comments"]) || responses.schemaVersion !== 1
...
function parse(item: SyncItem): Payload | undefined {
  ...
  if (!exact(p, keys) || !uuid.test(String(p.clientUuid)) || p.clientUuid !== item.entityId || !uuid.test(String(p.jobId))
    || p.systemKey !== "fire_intercom" || p.instanceKey !== "primary" || p.configuredZoneId !== null || p.configuredLocationId !== null || p.displaySequence !== 1
...
/** A V7 Fire Intercom payload is a `masterSystemInspection` frozen to template
 * version 7. Fire Intercom has no historical (V1-V6) sync path at all - see
 * masterServiceReportV7.ts. */
export function isFireIntercomV7Payload(item: SyncItem) {
  return record(item.payload) && item.payload.systemKey === "fire_intercom" && record(item.payload.masterTemplate) && item.payload.masterTemplate.version === 7;
}

/** Fire Intercom has no historical (V1-V6) path to fall back to - every item
 * dispatched here is V7, unlike `syncHydrantInspections` and friends. */
export async function syncFireIntercomInspections(items: SyncItem[], actorUserId?: number): Promise<SyncResult> {
  const out: SyncResult = { acceptedIds: [], duplicateIds: [], failed: [] };
  for (const item of items) {
    const v7 = await acceptFireIntercomV7Inspection(item, actorUserId);
    out.acceptedIds.push(...v7.acceptedIds); out.duplicateIds.push(...v7.duplicateIds); out.failed.push(...v7.failed);
  }
  return out;
}

export async function acceptFireIntercomV7Inspection(item: SyncItem, actorUserId?: number): Promise<SyncResult> {
  ...
  if (!payload || !actorUserId) { result.failed.push(fail(id, "VALIDATION_ERROR", "V7 Fire Intercom operation is invalid")); return result; }
  ...
  try {
    ...
    const system = job && Array.isArray(job.configuration_snapshot.enabledSystems)
      ? job.configuration_snapshot.enabledSystems.find((value) => record(value) && value.systemKey === "fire_intercom" && value.definitionStatus === "confirmed")
      : undefined;
    const definition = job ? await client.query<{ definition: unknown; definition_status: string }>("SELECT definition,definition_status FROM master_service_report_systems WHERE template_version_id=$1 AND system_key='fire_intercom'", [payload.masterTemplate.id]) : undefined;
    const contractSha256 = definition?.rows[0] ? v7EvidenceContractSha256(definition.rows[0].definition) : "";
    const adapter = definition?.rows[0] ? resolveV7EvidenceContract({ systemKey: "fire_intercom", templateId: payload.masterTemplate.id, templateVersion: 7, definition: definition.rows[0].definition, contractSha256 }) : undefined;
    const manifest = adapter ? parseV7EvidenceManifest(payload.evidenceManifest, adapter, payload.responses) : undefined;
    const expectedRows = record(system) ? expectedConfiguredFireIntercomRows(system) : undefined;
    ...
    if (!rows) { await client.query("ROLLBACK"); result.failed.push(fail(payload.clientUuid, "CONTRACT_MISMATCH", "Frozen Fire Intercom V7 contract is invalid")); return result; }
    if (!validResponses(payload.responses, rows) || !configuredFireIntercomRowsMatch(payload.responses, expectedRows)) { await client.query("ROLLBACK"); result.failed.push(fail(payload.clientUuid, "VALIDATION_ERROR", "V7 Fire Intercom responses are invalid")); return result; }
    ...
    await client.query("INSERT INTO master_system_inspections(id,job_id,system_key,created_by_user_id) VALUES($1,$2,'fire_intercom',$3) ON CONFLICT(job_id,system_key) DO NOTHING", [groupId, payload.jobId, actorUserId]);
    const group = await client.query<{ id: string }>("SELECT id FROM master_system_inspections WHERE job_id=$1 AND system_key='fire_intercom' FOR UPDATE", [payload.jobId]);
    if (!group.rowCount) throw new Error("V7 Fire Intercom group unavailable");
    const snapshot = { schemaVersion: 2, acceptedAt: new Date().toISOString(), ...authority, system: { ...system, key: "fire_intercom", definition: definition.rows[0]!.definition, repetitionMode: "single_with_repeatable_rows" }, instance: { instanceKey: "primary", displaySequence: 1, zone: null, location: null }, evidenceManifest: manifest };
