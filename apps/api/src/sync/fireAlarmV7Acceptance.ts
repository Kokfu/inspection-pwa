...
function validResponses(responses: Value, controls: ReturnType<typeof resolveFireAlarmV6Controls>) {
  ...
  const checkGroup = (group: unknown, definitions: readonly { key: string; result: { options: Array<{ value: string }> } }[]) => record(group) && exact(group, definitions.map((item) => item.key)) && definitions.every((definition) => {
    const field = group[definition.key];
    return record(field) && exact(field, ["result", "remarks"]) && definition.result.options.some((option) => option.value === field.result)
      && typeof field.remarks === "string" && field.remarks.length <= 2000;
  });
  if (!checkGroup(responses.chargerAndBatteries, controls.chargerAndBatteries) || !checkGroup(responses.mainFunctionKeys, controls.mainFunctionKeys)) return false;
  ...
  for (let index = 0; index < responses.secondaryAlarmDeviceRows.length; index += 1) {
    ...
    if (!record(row) || !exact(row, keys) || typeof row.rowUuid !== "string" || !uuid.test(row.rowUuid) || ids.has(row.rowUuid) || row.displaySequence !== index + 1 || typeof row.assetReference !== "string" || row.assetReference.length > 250 || typeof row.location !== "string" || !row.location.trim() || row.location.length > 300 || typeof row.remarks !== "string" || row.remarks.length > 2000 || !controls.secondaryAlarmDeviceRows.alarmBell.options.some((option) => option.value === row.alarmBell) || !controls.secondaryAlarmDeviceRows.manualCallPoint.options.some((option) => option.value === row.manualCallPoint) || !record(row.fieldRemarks) || Object.keys(row.fieldRemarks).some((key) => key !== "alarmBell" && key !== "manualCallPoint") || Object.values(row.fieldRemarks).some((value) => typeof value !== "string" || value.length > 2000)) return false;
    if (isV7EvidenceFinding(row.alarmBell) && !String(row.fieldRemarks.alarmBell ?? "").trim()) return false;
    if (isV7EvidenceFinding(row.manualCallPoint) && !String(row.fieldRemarks.manualCallPoint ?? "").trim()) return false;
    ids.add(row.rowUuid);
  }
  return true;
}
...
export async function acceptFireAlarmV7Inspection(item: SyncItem, actorUserId?: number): Promise<SyncResult> {
  ...
  if (!payload || !actorUserId) { result.failed.push(fail(id, "VALIDATION_ERROR", "V7 Fire Alarm operation is invalid")); return result; }
  ...
  try {
    ...
    if (committed.rowCount) {
      ...
      else if (snapshotJob?.id === payload.jobId && snapshotConfiguration?.revisionId === payload.configuration.revisionId && snapshotConfiguration?.revisionNumber === payload.configuration.revisionNumber && snapshotTemplate?.id === payload.masterTemplate.id && snapshotTemplate?.version === 7 && canonical(existing.response_payload) === canonical(payload.responses) && snapshotManifest && sameManifest(snapshotManifest, payload.evidenceManifest) && existing.performed_at.toISOString() === payload.performedAt) result.duplicateIds.push(payload.clientUuid);
    ...
    if (!job || !configuration || !template || !record(system) || definition?.rowCount !== 1 || definition.rows[0]!.definition_status !== "confirmed" || !adapter || job.master_template_version_id !== payload.masterTemplate.id || job.customer_configuration_revision_id !== payload.configuration.revisionId || template.id !== payload.masterTemplate.id || template.code !== "MFE-FSSR" || template.version !== 7 || configuration.revisionId !== payload.configuration.revisionId || configuration.revisionNumber !== payload.configuration.revisionNumber || !expectedConfiguredRows(system)) { await client.query("ROLLBACK"); result.failed.push(unavailable(payload.clientUuid)); return result; }
    let controls: ReturnType<typeof resolveFireAlarmV6Controls>;
    try { controls = resolveFireAlarmV6Controls(definition.rows[0]!.definition, 7); } catch { await client.query("ROLLBACK"); result.failed.push(fail(payload.clientUuid, "CONTRACT_MISMATCH", "Frozen Fire Alarm V7 contract is invalid")); return result; }
    if (!validResponses(payload.responses, controls)) { await client.query("ROLLBACK"); result.failed.push(fail(payload.clientUuid, "VALIDATION_ERROR", "V7 Fire Alarm responses are invalid")); return result; }
    // The manifest describes the technician's own payload, never whether this Job
    // exists, so it is reported only after the collapsed job-access guard above
    // has already passed.  Folding it into that guard (as this did until G7) told
    // a technician who attached one photo to two findings that the *inspection*
    // was unavailable, which is both untrue and unactionable.
    ...
    if ((manifest.length > 0 || reserved) && (!reserved || reserved.job_id !== payload.jobId || reserved.system_key !== payload.systemKey || reserved.master_template_version_id !== payload.masterTemplate.id || reserved.master_template_version !== 7 || reserved.system_contract_sha256 !== contractSha256 || reserved.reserved_by_user_id !== String(actorUserId))) { await client.query("ROLLBACK"); result.failed.push(unavailable(payload.clientUuid)); return result; }
    ...
    // The manifest check above cannot see that, and the accepted-evidence unique
    // indexes would reject it later as an opaque conflict.
