...
type HoseReelPayload = { clientUuid: string; jobId: string; systemKey: "hose_reel"; originalCreatorSnapshot: CreatorSnapshot | null; masterTemplate: { id: string; code: "MFE-FSSR"; version: number }; configuration: { revisionId: string; revisionNumber: number }; inspectionSnapshot: UnknownRecord; responses: UnknownRecord; performedAt: string };
...
function validateItem(item: SyncRequestItem): { payload?: HoseReelPayload; failure?: SyncFailure } {
  ...
  if (!isUuid(payload.clientUuid) || payload.clientUuid !== item.entityId || !isUuid(payload.jobId) || payload.systemKey !== "hose_reel" || !isRecord(payload.masterTemplate) || !isUuid(payload.masterTemplate.id) || payload.masterTemplate.code !== "MFE-FSSR" || !Number.isSafeInteger(payload.masterTemplate.version) || Number(payload.masterTemplate.version) < 1 || !isRecord(payload.configuration) || !isUuid(payload.configuration.revisionId) || !Number.isInteger(payload.configuration.revisionNumber) || !isRecord(payload.inspectionSnapshot) || !validCreatorSnapshot(payload.originalCreatorSnapshot) || !isRecord(payload.responses) || !isTimestamp(payload.performedAt)) return { failure: failure(id, "VALIDATION_ERROR", "Master system inspection payload is invalid") };
...
function enabledSystem(snapshot: UnknownRecord) {
  ...
  const found = systems.find((system) => isRecord(system) && system.systemKey === "hose_reel" && system.definitionStatus === "confirmed");
...
export async function syncMasterSystemInspections(items: SyncRequestItem[], actorUserId?: number): Promise<SyncResult> {
  ...
  for (const item of items) {
    ...
    try {
      ...
      const canonicalSnapshot = { schemaVersion: 1, acceptedAt, job: { id: payload.jobId, reference: jobRow.job_reference, title: jobRow.title }, customer: jobRow.configuration_snapshot.customer, configuration: jobRow.configuration_snapshot.configuration, template: { id: payload.masterTemplate.id, code: "MFE-FSSR", version: payload.masterTemplate.version }, system: { ...system, definition: definition.rows[0].definition, resolvedControls: controls, repetitionMode: "single_with_repeatable_rows", drumTypeCardinality: "pending_confirmation" } };
