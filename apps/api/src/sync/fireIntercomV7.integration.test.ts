...
test("Fire Intercom V7 accepted inspection completes into a Final Report carrying its bound photo", { skip: !databaseUrl }, async () => {
  ...
  try {
    await withV7Database(async (database) => {
      ...
      assert.deepEqual(outcome.acceptedIds, [clientUuid], JSON.stringify(outcome));
      await database.query("UPDATE inspection_jobs SET status='closed', completed_at=$2, completed_by_user_id=$3, completed_by_display_name='Fire Intercom V7 closer', report_number='TEST/' || id::text, technician_team_snapshot='[]'::jsonb WHERE id=$1", [job, at, fixture.actor]);
