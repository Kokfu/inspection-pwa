...
test("FM200 V7 stages distinct multipart evidence and atomically binds it to its own location form, independent of CO2", { skip: !databaseUrl }, async (t) => {
  ...
  try {
    ...
    const app = express(); app.use(express.json()); app.use((request, _response, next) => { const requested = request.header("x-test-actor"); if (requested !== "none") { const userId = requested === "admin" ? adminActor : requested === "foreign" ? foreignActor : actor; request.currentUser = { id: userId, username: "fm200", role: requested === "admin" ? "admin" : "inspector" }; } next(); }); app.use(stagedEvidenceRouter); app.use(masterSystemInspectionsRouter); app.use(syncRouter);
