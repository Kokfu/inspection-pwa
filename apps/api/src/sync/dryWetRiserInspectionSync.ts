...
export async function syncDryWetRiserInspections(items: Item[], actorUserId?: number): Promise<SyncResult> {
  ...
  for (const item of items) {
    ...
    try {
      ...
      if (!job || !system || !frozenConfiguration || !frozenTemplate || job.master_template_version_id !== payload.masterTemplate.id || job.customer_configuration_revision_id !== payload.configuration.revisionId || frozenConfiguration.revisionId !== payload.configuration.revisionId || frozenConfiguration.revisionNumber !== payload.configuration.revisionNumber || frozenTemplate.id !== payload.masterTemplate.id || frozenTemplate.code !== "MFE-FSSR" || frozenTemplate.version !== payload.masterTemplate.version || authoritative.riserMode !== frozen.riserMode) { await client.query("ROLLBACK"); result.failed.push(failure(id, "VALIDATION_ERROR", "Dry/Wet Riser job configuration is unavailable")); continue; }
      ...
      if (!definition || !isCompatibleSystemContract("dry_wet_riser", definition.definition_status, definition.definition)) { await client.query("ROLLBACK"); result.failed.push(failure(id, "VALIDATION_ERROR", "Dry/Wet Riser definition is unavailable")); continue; }
      ...
      if (job.status !== "open") { await client.query("ROLLBACK"); result.failed.push(failure(id, "JOB_CLOSED", "Inspection job is completed")); continue; }
