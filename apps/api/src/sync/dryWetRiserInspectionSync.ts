...
function creator(value: unknown) { return value === null || (rec(value) && exact(value, ["source", "userId", "username", "role", "capturedAt"]) && value.source === "device_reported" && Number.isInteger(value.userId) && typeof value.username === "string" && value.username.length <= 160 && (value.role === "admin" || value.role === "inspector") && typeof value.capturedAt === "string" && !Number.isNaN(Date.parse(value.capturedAt))); }
...
export async function syncDryWetRiserInspections(items: Item[], actorUserId?: number): Promise<SyncResult> {
  ...
  for (const item of items) {
    ...
    try {
      ...
      const fingerprint = createHash("sha256").update(canonical({ clientUuid:id, jobId:payload.jobId, systemKey:"dry_wet_riser", instanceKey:"primary", configuredZoneId:null, configuredLocationId:null, displaySequence:1, masterTemplate:frozenTemplate, configuration:frozenConfiguration, inspectionSnapshot:snapshotForFingerprint, responses, performedAt:payload.performedAt, originalCreatorSnapshot:payload.originalCreatorSnapshot, actorUserId:actorUserId ?? null })).digest("hex");
