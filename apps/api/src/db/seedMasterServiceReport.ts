...
type MasterServiceReportTemplate = typeof masterServiceReportV1 | typeof masterServiceReportV2 | typeof masterServiceReportV3 | typeof masterServiceReportV4 | typeof masterServiceReportV5 | typeof masterServiceReportV6 | typeof masterServiceReportV7;
...
export async function assertDryWetRiserFixture(client: PoolClient, snapshot: unknown) {
  ...
  // A configuration revision may be reused by any number of later Service
  // Visits. The deterministic fixture Job is asserted by its fixed ID below;
  // it must not make the shared revision exclusive to that Job.
  const membership = await client.query<{ enabled: number; zones: number; locations: number }>(`SELECT (SELECT count(*)::int FROM customer_enabled_systems WHERE configuration_revision_id=$1) AS enabled,(SELECT count(*)::int FROM customer_system_zones WHERE enabled_system_id=$2) AS zones,(SELECT count(*)::int FROM customer_system_locations WHERE enabled_system_id=$2) AS locations`, [demoRiserRevisionId, demoRiserEnabledSystemId]);
  if (!membership.rows[0] || membership.rows[0].enabled !== 1 || membership.rows[0].zones !== 0 || membership.rows[0].locations !== 2) throw new Error("Dry Wet Riser fixture has unexpected members");
