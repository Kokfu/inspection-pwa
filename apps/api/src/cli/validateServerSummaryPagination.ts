...
async function insertAccepted(c: Case, clientUuid: string, actor: number, instanceKey = "primary") {
...
async function readAll(cookie: string) {
  const records: any[] = []; const sizes: number[] = []; let cursor: string | undefined;
  do { const url = new URL("http://localhost:3000/master-system-inspections"); if (cursor) url.searchParams.set("cursor",cursor);
    const r = await fetch(url,{headers:{Cookie:cookie}}); assert(r.status===200,`summary HTTP ${r.status}`); const body:any=await r.json(); assert(Array.isArray(body.inspections)&&typeof body.hasMore==="boolean"&&("nextCursor" in body),"bad cursor envelope"); records.push(...body.inspections); sizes.push(body.inspections.length); cursor=body.hasMore ? body.nextCursor : undefined; assert(!body.hasMore || typeof cursor === "string","missing next cursor");
  } while(cursor); return {records,sizes};
}
async function main() { try { await cleanup(); const cookie=await login(); const actor=(await pool.query<{id:number}>("SELECT id FROM users WHERE username=$1",[username])).rows[0].id;
  ...
  for(let n=1;n<=101;n++){const c=await createCase("dry_wet_riser","00000000-0000-4000-8000-000000000819");const id=uuid(n);await insertAccepted(c,id,actor);expected.push(id);}
  ...
  assert(validationIds.length===105 && new Set(validationIds).size===105 && validationIds.every((id,i)=>id===ordered[i]),"summary skipped, duplicated, or unordered UUID"); assert(JSON.stringify(ids)===JSON.stringify(second.records.map(x=>x.clientUuid)),"complete read order was unstable");
  assert(first.sizes[0]===100&&first.sizes[1]>=5,"pagination did not cross 100-row boundary");
  ...
  report.summariesCreated=105; report.pageSizes=first.sizes; report.noSkipNoDuplicate=true; report.deterministicTieBreak=true; report.hoseReelBeyond100=true; report.dryWetBeyond100=true; report.co2SplitAcrossPages=true; report.emptyCompleteVsIncomplete="endpoint envelope distinguishes hasMore=false from continuation";
