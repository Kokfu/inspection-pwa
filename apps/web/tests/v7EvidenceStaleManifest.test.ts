...
const suppressionResponses = (resultA: "poor" | "good" | "not_relevant", cylinder: string): any => ({ chargerAndBatteries: { main_supply: { result: resultA, remarks: "A" }, battery: { result: "good", remarks: "" }, charger: { result: "good", remarks: "" } }, physicalOutlook: { [cylinder]: { result: "poor", remarks: "B" } }, mainFunctionKeys: { lamp_test: { result: "good", remarks: "" } } });
const fireResponses = (resultA: "poor" | "good" | "not_relevant"): any => ({ chargerAndBatteries: { main_supply: { result: resultA, remarks: "A" }, battery: { result: "good", remarks: "" }, charger: { result: "good", remarks: "" } }, mainFunctionKeys: { main_alarm_reset: { result: "good", remarks: "" } }, secondaryAlarmDeviceRows: [{ rowUuid: "00000000-0000-4000-8000-000000000099", alarmBell: "poor", manualCallPoint: "good", fieldRemarks: { alarmBell: "B" } }] });
...
test("V7 stale-draft evidence matrix keeps only current Poor fields and freezes Pending authority", () => {
  for (const [system, cylinder] of [["CO2", "co2_cylinder"], ["Wet Chemical", "wet_chemical_cylinder"]] as const) {
    ...
    for (const next of ["good", "not_relevant"] as const) {
    ...
    const pending = structuredClone({ responses: suppressionResponses("poor", cylinder), evidenceManifest: v7EvidenceManifest(photos as never, suppressionResponses("poor", cylinder)) });
    assert.equal(pending.responses.chargerAndBatteries.main_supply.result, "poor", `${system} frozen Pending response is immutable under later Draft edits`);
  ...
  const firePending = structuredClone({ responses: fireResponses("poor"), evidenceManifest: v7FireAlarmManifest(firePhotos as never, fireResponses("poor")) });
  assert.equal(firePending.responses.chargerAndBatteries.main_supply.result, "poor");
