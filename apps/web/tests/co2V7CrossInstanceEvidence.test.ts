...
} from "../src/co2/v7Evidence";
import type { Co2Responses, MasterSystemFormInstanceRecord } from "../src/co2/co2Types";
import type { InspectionAttachmentRecord } from "../src/attachments/attachmentTypes";

// G9.  One CO2/Wet Chemical Job holds several location-instances of the same
// system, each its own masterSystemFormInstances row.  Server evidence
// uniqueness is jobId+systemKey, so the same photo on Location A and Location B
// is refused at acceptance with a *retryable* EVIDENCE_CONFLICT — and the outbox
...
function responses(overrides: Partial<Co2Responses> = {}): Co2Responses {
  return {
...
function record(values: {
  ...
  } as unknown as MasterSystemFormInstanceRecord;
}
...
function source(instances: MasterSystemFormInstanceRecord[], attachments: InspectionAttachmentRecord[]): V7EvidenceSource {
...
const JOB = "job-1";
const CO2_GROUP = `${JOB}:co2_fire_extinguisher`;
const WET_GROUP = `${JOB}:wet_chemical`;
const OTHER_JOB_GROUP = "job-2:co2_fire_extinguisher";

const locationA = record({ clientUuid: "a", groupKey: CO2_GROUP, displaySequence: 1, locationName: "CO2 Room A" });
const locationB = record({ clientUuid: "b", groupKey: CO2_GROUP, displaySequence: 2, locationName: "CO2 Room B" });
...
test("the same photo on a finding in two locations of one Job is refused before it can be queued", async () => {
  ...
  assert.ok(
    issues.some((issue) => issue.includes("CO2 Room A")),
...
test("the capture-time guard refuses the same bytes and names the other location", async () => {
  ...
  assert.deepEqual(crossInstanceV7PhotoClash("same-bytes", siblings), {
    locationLabel: "CO2 Room A", fieldPath: MAIN_SUPPLY, sha256: "same-bytes"
  });
...
test("a sibling photo left on a field that is no longer a finding does not block", async () => {
  ...
  const staleA = record({ clientUuid: "a", groupKey: CO2_GROUP, displaySequence: 1, locationName: "CO2 Room A" });
...
test("a different system key in the same Job may reuse the same bytes", async () => {
  const wet = record({ clientUuid: "w", groupKey: WET_GROUP, displaySequence: 1, locationName: "Kitchen" });
  const attachments = [attachment("w", MAIN_SUPPLY, "same-bytes"), attachment("b", MAIN_SUPPLY, "same-bytes")];
  const siblings = await collectV7SiblingEvidence(locationB, source([wet, locationB], attachments));
