...
const pumpLayout: SprinklerLayoutRow[] = [
...
const mainValveLayout: SprinklerLayoutRow[] = [
...
export function resolvePublishedAutomaticSprinklerControls(
  definition: unknown,
...
export function controlsForAutomaticSprinklerSnapshot(snapshot: {
  template: { code: string; version: number };
  system: { definition: unknown; resolvedControls?: unknown };
}): ResolvedAutomaticSprinklerControls {
  const frozen = snapshot.system.resolvedControls;
  const expected = resolvePublishedAutomaticSprinklerControls(
    snapshot.system.definition,
    snapshot.template.code,
    snapshot.template.version
  );
  if (!isRecord(frozen) || JSON.stringify(frozen) !== JSON.stringify(expected)) {
    throw new Error("Frozen Automatic Sprinkler controls do not match the frozen definition");
  }
  return frozen as unknown as ResolvedAutomaticSprinklerControls;
}
