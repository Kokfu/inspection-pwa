...
export async function runDryWetRiserServerDetailHarness(mount: HTMLElement) {
  ...
  const mutations: [string, (value: any) => void][] = [
    ["missing clientUuid", (v) => delete v.clientUuid], ["malformed clientUuid", (v) => v.clientUuid = "bad"], ["wrong systemKey", (v) => v.systemKey = "hose_reel"], ["invalid template version", (v) => v.template.version = 0], ["missing systemConfiguration", (v) => delete v.systemConfiguration], ["stored mode contradiction", (v) => v.responses.mode = "wet"], ["invalid riserMode", (v) => v.systemConfiguration.riserMode = "other"], ["extra systemConfiguration key", (v) => v.systemConfiguration.extra = true], ["missing Water Tank member", (v) => delete v.responses.waterTank.water_level], ["extra Water Tank member", (v) => v.responses.waterTank.extra = { result: "good", remarks: "" }], ["missing Pump House member", (v) => delete v.responses.pumpHouse.pump_house_clean], ["extra Pump House member", (v) => v.responses.pumpHouse.extra = { result: "good", remarks: "" }], ["missing PSI value", (v) => delete v.responses.measurements.jockeyCutIn], ["non-finite PSI", (v) => v.responses.measurements.jockeyCutIn = Infinity], ["wrong PSI unit", (v) => v.responses.measurements.unit = "BAR"], ["empty outlets", (v) => v.responses.riserOutlets = []], ["malformed outlet UUID", (v) => v.responses.riserOutlets[0].rowUuid = "bad"], ["duplicate outlet UUID", (v) => v.responses.riserOutlets.push({ ...v.responses.riserOutlets[0], sortOrder: 2 })], ["duplicate sortOrder", (v) => v.responses.riserOutlets.push({ ...v.responses.riserOutlets[0], rowUuid: uuid("000000000010"), sortOrder: 1 })], ["non-contiguous sortOrder", (v) => v.responses.riserOutlets[0].sortOrder = 2], ["malformed configured location snapshot", (v) => v.responses.riserOutlets[0].locationSnapshot.id = uuid("000000000011")], ["configured location text contradiction", (v) => v.responses.riserOutlets[0].locationText = "forged"], ["malformed configured zone snapshot", (v) => v.responses.riserOutlets[0].zoneSnapshot.extra = true], ["technician location snapshot", (v) => Object.assign(v.responses.riserOutlets[0], { source:"technician", configuredLocationId:null, configuredRowOrdinal:null, assetReference:"", zoneSnapshot:null })], ["missing outlet result", (v) => delete v.responses.riserOutlets[0].doorResult], ["invalid Good/Poor result", (v) => v.responses.riserOutlets[0].doorResult = "unknown"], ["malformed remarks", (v) => v.responses.riserOutlets[0].remarks = 42], ["malformed comments", (v) => v.responses.comments = 42], ["unsupported top-level key", (v) => v.extra = true]
  ];
