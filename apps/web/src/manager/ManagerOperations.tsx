...
export function ManagerOperations({ visits, loading, message, selectedVisit, acceptedRecords = [], acceptedRecordsUnavailable = false, backLabel = "Back to Operations", supervisor = false, onRefresh, onSelect, onBack, onViewReport, onDownloadReport, onCorrect }: {
  ...
  if (selectedVisit) return <section className="manager-home" aria-labelledby="manager-detail-title">
    ...
    {selectedVisit.status === "closed" ? <section className="report-summary"><h3>Completion</h3><p>Completed {selectedVisit.completion.completedAt ? formatMalaysiaDateTime(selectedVisit.completion.completedAt) : "at an unknown time"} by {selectedVisit.completion.completedBy?.username ?? "the recorded technician"}.</p><button type="button" onClick={() => onViewReport(selectedVisit)}>View Final Report</button></section> : <p className="offline-notice">This open service visit is read-only in this workspace.</p>}
  </section>;
  ...
  return <section className="manager-home" aria-labelledby="manager-home-title"><div className="home-toolbar"><div><p className="eyebrow">{supervisor ? "Supervisor workspace" : "Manager workspace"}</p><h2 id="manager-home-title">Operations</h2><p>Read-only operational service visibility.</p></div><button type="button" className="secondary-command" disabled={loading} onClick={() => void onRefresh()}>{loading ? "Refreshing…" : "Refresh"}</button></div>
