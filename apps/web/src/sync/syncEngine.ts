...
async function syncPendingV7Evidence() {
  ...
  for (const item of work) {
    ...
    const record = attachment?.systemKey === "fire_alarm_detector" || attachment?.systemKey === "hydrant" || attachment?.systemKey === "hose_reel" || attachment?.systemKey === "automatic_sprinkler" || attachment?.systemKey === "dry_wet_riser" || attachment?.systemKey === "smoke_ventilation" ? await localDatabase.masterSystemInspections.get(attachment.inspectionClientUuid) : attachment ? await localDatabase.masterSystemFormInstances.get(attachment.inspectionClientUuid) : undefined;
    if (!attachment || attachment.protocolVersion !== 7 || !record || attachment.systemKey !== record.systemKey || (record.systemKey !== "co2_fire_extinguisher" && record.systemKey !== "wet_chemical" && record.systemKey !== "fire_alarm_detector" && record.systemKey !== "hydrant" && record.systemKey !== "hose_reel" && record.systemKey !== "automatic_sprinkler" && record.systemKey !== "dry_wet_riser" && record.systemKey !== "smoke_ventilation")) { await localDatabase.syncOutbox.update(item.operationId, { status: "Failed", lastError: "Local V7 evidence or inspection is unavailable" }); failed += 1; continue; }
    const at = new Date().toISOString();
    await localDatabase.transaction("rw", localDatabase.inspectionAttachments, localDatabase.syncOutbox, async () => { await localDatabase.inspectionAttachments.update(attachment.photoUuid, { syncStatus: "Uploading", localUpdatedAt: at, lastSyncError: undefined }); await localDatabase.syncOutbox.update(item.operationId, { status: "Syncing", attempts: item.attempts + 1, lastAttemptAt: at, lastError: undefined }); });
    try {
...
async function v7FormReady(item: SyncOutboxItem) {
  const isV7MasterSystemInspectionOutboxItem = item.entityType === "masterSystemInspection" && typeof item.payload === "object" && item.payload !== null && (["hydrant", "hose_reel", "automatic_sprinkler", "dry_wet_riser", "smoke_ventilation"] as const).some((systemKey) => (item.payload as { systemKey?: unknown }).systemKey === systemKey);
