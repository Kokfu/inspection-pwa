...
import { loadV7AcceptedEvidence, v7AcceptedEvidenceContentUrl, type V7Fm200AcceptedEvidence } from "./fm200AcceptedEvidence";
...
export function ServerFm200View({ inspection, onBack }: { inspection: ServerFm200Detail; onBack: () => void }) {
  const responses = inspection.responses;
  const controls = inspection.displayControls;
  const [evidence, setEvidence] = useState<V7Fm200AcceptedEvidence[]>([]);
  const [evidenceError, setEvidenceError] = useState("");
  useEffect(() => {
    let current = true;
    if (inspection.template.version !== 7) { setEvidence([]); setEvidenceError(""); return () => { current = false; }; }
    void loadV7AcceptedEvidence(inspection.clientUuid, "fm200_fire_suppression").then((items) => { if (current) { setEvidence(items); setEvidenceError(""); } }, () => { if (current) setEvidenceError("Accepted evidence is unavailable."); });
  ...
  return <section className="hose-reel-form server-inspection-detail">
    <button type="button" className="secondary-command" onClick={onBack}>Back to FM200 Locations</button>
