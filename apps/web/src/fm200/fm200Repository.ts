...
import { resolvePublishedFm200Controls, type ResolvedFm200Controls } from "./fm200Definition";
import type {
  Fm200ConfiguredInstance,
  Fm200DetectorRow,
  Fm200InspectionSnapshot,
  Fm200MasterSystemFormInstanceRecord,
  Fm200MasterSystemInspectionGroupRecord,
  Fm200Responses,
  Fm200SystemKey
} from "./fm200Types";
import { collectV7SiblingEvidence, listV7SuppressionPhotos, v7EvidenceManifest, v7EvidenceOutbox, v7SubmissionIssues } from "./fm200Evidence";

const fm200SystemKey: Fm200SystemKey = "fm200_fire_suppression";
const uuidPattern = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;
const now = () => new Date().toISOString();
export const fm200GroupKey = (jobId: string) => `${jobId}:${fm200SystemKey}`;
export type Fm200SubmitIssue = { section: string; message: string; targetId: string };
...
function definitionFor(catalog: InspectionCatalogInput, job: InspectionJob) {
  const compatible = "templates" in catalog
    ? compatibleCatalogSystem(catalog, job.configurationSnapshot.template, fm200SystemKey)
    : undefined;
  ...
  const system = compatible?.system ?? template.systems.find((candidate) => candidate.key === fm200SystemKey && candidate.definitionStatus === "confirmed");
  if (!system?.definition) throw new Error("Cached FM200 definition is unavailable. Refresh jobs online first.");
  return {
    definition: system.definition,
    controls: resolvePublishedFm200Controls(system.definition, template.code, template.version)
  };
}

function expectedInstances(system: JobSystemSnapshot): Fm200ConfiguredInstance[] {
...
function blankDetectorRow(instance: Fm200ConfiguredInstance): Fm200DetectorRow {
  return {
...
function checklist(definitions: ResolvedFm200Controls["chargerAndBatteries"]) {
  return Object.fromEntries(definitions.map((definition) => [definition.key, { result: null, remarks: "" }]));
}

function blankResponses(instance: Fm200ConfiguredInstance, controls: ResolvedFm200Controls): Fm200Responses {
  return {
...
function snapshot(
  ...
  instance: Fm200ConfiguredInstance,
  definition: unknown,
  controls: ResolvedFm200Controls,
  capturedAt: string
): Fm200InspectionSnapshot {
...
export async function initializeFm200InspectionGroup(
  job: InspectionJob,
  system: JobSystemSnapshot,
  catalog: InspectionCatalogInput,
  creator: { id: number; username: string; role: "admin" | "inspector" } | undefined
) {
  if (system.systemKey !== fm200SystemKey) throw new Error("Selected system is not a supported suppression system");
  ...
  try {
    await localDatabase.transaction(
      ...
      async () => {
        ...
        const group: Fm200MasterSystemInspectionGroupRecord = (existingGroup as Fm200MasterSystemInspectionGroupRecord | undefined) ?? {
          groupKey,
          jobId: job.id,
          systemKey,
        ...
        for (const instance of group.expectedInstances) {
          ...
          const record: Fm200MasterSystemFormInstanceRecord = {
            schemaVersion: 1,
    ...
    const racedInstances = await listFm200Instances(groupKey);
  ...
  if (!group) throw new Error("FM200 inspection group initialization did not complete");
  return { group: group as Fm200MasterSystemInspectionGroupRecord, instances: await listFm200Instances(groupKey) };
}

export function listFm200Instances(groupKey: string) {
  return localDatabase.masterSystemFormInstances.where("groupKey").equals(groupKey).sortBy("displaySequence") as Promise<Fm200MasterSystemFormInstanceRecord[]>;
}
...
function canonicalV7Responses(record: Fm200MasterSystemFormInstanceRecord, responses: Fm200Responses): Fm200Responses {
  ...
  return { ...responses, detectorRows: responses.detectorRows.map((row) => ({ ...row,
    heatDetectorStatus: canonicalV7DetectorStates(controls.detectorRows.heatDetector.result, row.heatDetectorStatus) as Fm200DetectorRow["heatDetectorStatus"],
    smokeDetectorStatus: canonicalV7DetectorStates(controls.detectorRows.smokeDetector.result, row.smokeDetectorStatus) as Fm200DetectorRow["smokeDetectorStatus"]
  })) };
}

export function getFm200SubmitIssues(record: Fm200MasterSystemFormInstanceRecord, responses: Fm200Responses): Fm200SubmitIssue[] {
  const controls = record.inspectionSnapshot.system.resolvedControls;
  const issues: Fm200SubmitIssue[] = [];
  if (!responses.controlPanelLocation.trim()) issues.push({ section: "FM200 Control Panel", message: "Control Panel Location is required", targetId: "fm200-panel-location" });
  if (responses.controlPanelLocation.length > controls.controlPanelLocation.maxLength) issues.push({ section: "FM200 Control Panel", message: `Control Panel Location exceeds ${controls.controlPanelLocation.maxLength} characters`, targetId: "fm200-panel-location" });
  if (responses.detectorRows.length < controls.detectorRows.minimum) issues.push({ section: "Detector Table", message: "At least one detector row is required", targetId: "fm200-detectors" });
  if (responses.detectorRows.length > controls.detectorRows.maximum) issues.push({ section: "Detector Table", message: `No more than ${controls.detectorRows.maximum} detector rows may be submitted`, targetId: "fm200-detectors" });
  const rowIds = new Set<string>();
  const displaySequences = new Set<number>();
  responses.detectorRows.forEach((row, index) => {
    const targetId = `fm200-detector-${row.rowUuid}`;
  ...
  groups.forEach(([section, definitions, values]) => definitions.forEach((definition) => {
    ...
    const targetId = `fm200-check-${definition.key}`;
  ...
  if (responses.comments.length > controls.comments.maxLength) issues.push({ section: "Comments", message: `Comments exceed ${controls.comments.maxLength} characters`, targetId: "fm200-comments" });
  return issues;
}

function updated(record: Fm200MasterSystemFormInstanceRecord, responses: Fm200Responses, status: Fm200MasterSystemFormInstanceRecord["syncStatus"]) {
  const timestamp = now();
  return {
    ...record,
    responses,
...
export async function saveFm200Draft(record: Fm200MasterSystemFormInstanceRecord, responses: Fm200Responses) {
  let next: Fm200MasterSystemFormInstanceRecord | undefined;
  await localDatabase.transaction("rw", localDatabase.masterSystemFormInstances, async () => {
    const live = await localDatabase.masterSystemFormInstances.get(record.clientUuid) as Fm200MasterSystemFormInstanceRecord | undefined;
    if (!live || live.systemKey !== record.systemKey || live.syncStatus !== "Draft" || live.localUpdatedAt !== record.localUpdatedAt) {
      throw new Error("Only the current Draft FM200 form can be edited");
    }
    next = updated(live, canonicalV7Responses(live, responses), "Draft");
    await localDatabase.masterSystemFormInstances.put(next);
  });
  if (!next) throw new Error("FM200 Draft was not saved");
  return next;
}

function payload(record: Fm200MasterSystemFormInstanceRecord, evidenceManifest?: ReturnType<typeof v7EvidenceManifest>) {
  return {
...
export async function submitLocalFm200(record: Fm200MasterSystemFormInstanceRecord, responses: Fm200Responses) {
  let next: Fm200MasterSystemFormInstanceRecord | undefined;
  await localDatabase.transaction("rw", localDatabase.masterSystemFormInstances, localDatabase.inspectionAttachments, localDatabase.syncOutbox, async () => {
    ...
    if (!live || live.systemKey !== record.systemKey || live.localUpdatedAt !== record.localUpdatedAt || live.syncStatus !== "Draft") {
      throw new Error("This FM200 form cannot be submitted in its current state");
    }
    ...
    if (getFm200SubmitIssues(live, canonicalResponses).length > 0 || v7SubmissionIssues(live, canonicalResponses, photos, siblings).length > 0) throw new Error("Complete the required FM200 fields and evidence before local submission");
  ...
  if (!next) throw new Error("FM200 form was not submitted locally");
  return next;
}

export async function returnFailedFm200ToDraft(record: Fm200MasterSystemFormInstanceRecord) {
  let next: Fm200MasterSystemFormInstanceRecord | undefined;
  ...
  await localDatabase.transaction("rw", localDatabase.masterSystemFormInstances, localDatabase.syncOutbox, async () => {
    ...
    if (!live || live.systemKey !== record.systemKey || live.localUpdatedAt !== record.localUpdatedAt || (live.syncStatus !== "Failed" && live.syncStatus !== "Conflict")) {
      throw new Error("This FM200 form changed elsewhere. Reload before correcting it.");
    }
  ...
  if (!next) throw new Error("FM200 form was not corrected");
  return next;
}

export function addFm200DetectorRow(responses: Fm200Responses) {
