...
import type { Fm200MasterSystemFormInstanceRecord, Fm200Responses } from "./fm200Types";

export type V7Fm200FieldPath =
...
const groups: Array<[keyof Pick<Fm200Responses, "chargerAndBatteries" | "physicalOutlook" | "mainFunctionKeys">, string]> = [
...
export function v7RequiredFieldPaths(responses: Fm200Responses) {
  const paths: V7Fm200FieldPath[] = [];
  for (const [group, prefix] of groups) for (const [key, value] of Object.entries(responses[group])) if (isEvidenceFinding(value.result)) paths.push(`${prefix}.${key}` as V7Fm200FieldPath);
  return paths.sort();
}
export function v7SubmissionIssues(record: Fm200MasterSystemFormInstanceRecord, responses: Fm200Responses, attachments: InspectionAttachmentRecord[], siblings: readonly V7SiblingEvidence[] = []) {
...
export function v7InstanceLocationLabel(record: Pick<Fm200MasterSystemFormInstanceRecord, "inspectionSnapshot" | "displaySequence">) {
 ...
 * (fm200Repository) and is indexed on masterSystemFormInstances (Dexie v9), as is
...
export type V7EvidenceSource = {
  masterSystemFormInstances: IndexedReader<Fm200MasterSystemFormInstanceRecord>;
  inspectionAttachments: IndexedReader<InspectionAttachmentRecord>;
};
export async function collectV7SiblingEvidence(record: Pick<Fm200MasterSystemFormInstanceRecord, "clientUuid" | "groupKey">, source: V7EvidenceSource = localDatabase as unknown as V7EvidenceSource) {
...
export async function saveV7SuppressionPhoto(values: { record: Fm200MasterSystemFormInstanceRecord; fieldPath: V7Fm200FieldPath; captureSource: AttachmentCaptureSource; blob: Blob; mimeType: "image/jpeg"; sizeBytes: number; width: number; height: number; sha256: string; capturedAt?: string }) {
  ...
  await localDatabase.transaction("rw", localDatabase.masterSystemFormInstances, localDatabase.inspectionAttachments, async () => {
    const live = await localDatabase.masterSystemFormInstances.get(values.record.clientUuid) as Fm200MasterSystemFormInstanceRecord | undefined;
...
export function v7EvidenceManifest(attachments: InspectionAttachmentRecord[], responses: Fm200Responses) { const currentPoor = new Set(v7RequiredFieldPaths(responses)); return attachments.filter((item) => item.protocolVersion === 7 && currentPoor.has(item.fieldPath as V7Fm200FieldPath)).map((item) => ({ photoUuid: item.photoUuid, fieldPath: item.fieldPath, sourceSha256: item.sha256 })).sort((a, b) => a.fieldPath.localeCompare(b.fieldPath)); }
